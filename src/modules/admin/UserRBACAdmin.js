    ...
    container.innerHTML = `
      <div>
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; flex-wrap: wrap; gap: 1rem;">
          <div>
            ...
            <p style="color: var(--text-muted); font-size: 0.9rem;">
              Manage platform administrators, assign roles (Super Admin, Admin, Sub-Admin), and scope granular access permissions.
            </p>
          </div>
        </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 1.5rem;">
          ${systemUsers.map(u => `
            <div class="glass-panel" style="padding: 1.75rem; border-radius: var(--radius-md); border-top: 4px solid ${u.role==='super_admin'?'var(--accent-gold)':u.role==='admin'?'var(--primary)':'#8b5cf6'};">
              <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; margin-bottom: 1.25rem;">
                <div>
                  <div style="display: flex; align-items: center; gap: 0.75rem;">
                    <h3 style="color: #fff; font-size: 1.2rem;">${escapeHtml(u.name)}</h3>
                    <span class="badge" style="background: rgba(255,255,255,0.08); color: ${u.role==='super_admin'?'var(--accent-gold)':'#fff'}; font-weight: 800;">
                      ${u.role.replace('_', ' ').toUpperCase()}
                    </span>
                  </div>
                  <div style="font-size: 0.85rem; color: var(--text-muted); margin-top: 0.2rem;">
                    ✉️ ${escapeHtml(u.email)} • 📍 ${escapeHtml(u.district)} District • 🕒 Last Login: ${u.lastLogin}
                  </div>
                </div>
    ...
    container.querySelectorAll('.req-perm-change-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        stateEngine.createApprovalRequest(
          ...
          `Administrator: ${btn.dataset.name}`,
          btn.dataset.id,
          'Modification of administrative role/permissions requested.',
          'HIGH',
          'super_admin'
        );
